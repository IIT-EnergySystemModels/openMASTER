...
def make_model():
    ...
    def EQ_PenalCost           (m, sYear ):
        return m.vPenalCost [sYear] ==  m.pYrGap * (1/((1+m.pDisRate)**(m.pYrGap*(m.sYear.ord(sYear)-1)))) * m.pESNSCost * (( 
                            ...
                            + m.pCECapExcess*sum(m.vCECapExc[sCE,sYear] for sCE in m.sCE)
                            + m.pSTCapExcess*sum(m.vSTCapExc[sST,sYear] for sST in m.sST))
    #G€
    ...
    def EQ_CELimit         (m, sCE,sYear        ):
        return m.pCEInsPathwayCap[sCE,sYear] >= m.vCETotCap[sCE,sYear] - m.vCECapExc [sCE,sYear]
    #GW
