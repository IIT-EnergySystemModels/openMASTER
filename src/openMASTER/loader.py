...
def load_dataportal_from_csv() -> DataPortal:
    ...
    for index, row in sets_and_parameters.iterrows():
        ...
        if   data_type == "Set":
             data.load(filename = os.path.join(PATH_MODEL_IN, sheetname, '.csv'), set = sheetname, format = "set")  
        elif data_type == "Parameter":
             data.load(filename = os.path.join(PATH_MODEL_IN, sheetname, '.csv'), index=index_names, param=[sheetname], format="table")
...
def _excel_table_to_list(path, sheetname, index_dim):
    ...
    out_filename = os.path.join(PATH_MODEL_IN, sheetname, '.csv')
